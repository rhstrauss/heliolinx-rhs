...
int main(int argc, char *argv[])
{
  ...
  status = link_purify_chisq_omp(image_log, detvec, clustvecmain, clust2detmain, config, n_workers, outclust, outclust2det);
  if(status!=0) {
    cerr << "ERROR: link_purify_chisq_omp failed with status " << status << "\n";
    return(status);
  } 
