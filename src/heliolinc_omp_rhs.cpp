...
int main(int argc, char *argv[])
{
  ...
  if(streaming) {
    status=heliolinc_alg_omp_rhs_streaming(image_log, detvec, tracklets, trk2det, radhyp, earthpos, config, sumfile, clust2detfile, bool(do_dedup));
    if(status!=0) {
      cerr << "ERROR: heliolinc_alg_omp_rhs_streaming failed with status " << status << "\n";
      return(status);
    }
  } else {
    status=heliolinc_alg_omp_rhs(image_log, detvec, tracklets, trk2det, radhyp, earthpos, config, sumfile, clust2detfile, bool(do_dedup));
    if(status!=0) {
