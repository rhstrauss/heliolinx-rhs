// make_trailed_tracklets_omp.cpp
//
// OpenMP-parallel wrapper around make_trailed_tracklets2() — the trailed
// (streak-aware) analogue of make_tracklets_omp.  Same night-parallel
// decomposition, but each night is processed by make_trailed_tracklets2()
// (which uses trail length + orientation, exposure time, and the
// cross-observatory parallax GCR fit) instead of make_tracklets7().
//
// Because pairing only joins images within [mintime, maxtime] (both
...
//
// New argument vs. make_trailed_tracklets:
//   -nw N          number of OpenMP worker threads (default 1 = serial,
//                  behaviour identical to make_trailed_tracklets)
//   -noon_local H  snap night boundaries to local noon at UTC offset H hours
//                  (e.g. -10 for Hawaii HST).  Default: MJD integer floor.
//
// All other arguments (including the trailed-only -siglenscale, -sigpascale,
// -exptime) are identical to make_trailed_tracklets.  The four output files
// (outimgs, pairdets, tracklets, trk2det) are written in exactly the same
// format.
//
// CAVEAT: night boundaries default to floor(MJD).  A tracklet whose two
// epochs straddle a boundary (within maxtime of UTC midnight) is assigned to
// different nights and thus not formed — so on multi-observatory global data
// the output can differ from the serial tool by a small number of
// boundary-straddling tracklets.  Use -noon_local to move the boundary into
// the observing gap of the dominant site to eliminate this for single-site data.
...
{
  cerr << "Usage: make_trailed_tracklets_omp -dets detfile -imgs imfile -outimgs output image file/ \n";
  ...
  cerr << "-minarc minimum total angular arc (arcsec)/ \n";
  cerr << "-siglenscale fractional trail length matching tolerance/ \n";
  cerr << "-sigpascale trail orientation matching tolerance (arcsec) -exptime exposure time (seconds)/ \n";
  cerr << "-earth earthfile -obscode obscodefile -forcerun\n";
  ...
  cerr << "\nor, at minimum\n\n";
  cerr << "make_trailed_tracklets_omp -dets detfile -earth earthfile -obscode obscodefile\n";
}
...
int main(int argc, char *argv[])
{
    ...
    if(string(argv[i]) == "-d" || string(argv[i]) == "-dets" || string(argv[i]) == "-det" || string(argv[i]) == "--dets" || string(argv[i]) == "--det" || string(argv[i]) == "--detection" || string(argv[i]) == "--detections") {
      ...
      if(i+1 < argc) {
        ...
        if(!isnormal(config.siglenscale) || config.siglenscale <= 0.0l) {
          cerr << "Error: invalid fractional trail length uncertainty parameter (" << config.siglenscale << ") supplied.\n";
          return(2);
        }
      } else {
        cerr << "Fractional trail length uncertainty parameter keyword supplied with no corresponding argument\n";
        show_usage();
        return(1);
      }
      ...
      if(i+1 < argc) {
        ...
        if(!isnormal(config.sigpascale) || config.sigpascale <= 0.0l) {
          cerr << "Error: invalid trail position angle uncertainty parameter (" << config.sigpascale << " arcsec) supplied.\n";
          return(2);
        }
      } else {
        cerr << "Trail position angle uncertainty parameter keyword supplied with no corresponding argument\n";
        show_usage();
        return(1);
      }
      ...
      if(i+1 < argc) {
        ...
        if(!isnormal(config.exptime) || config.exptime <= 0.0l) {
          cerr << "Error: invalid exposure time (" << config.exptime << " seconds) supplied.\n";
          return(2);
        }
      } else {
  ...
  cout << "Trailed params: siglenscale=" << config.siglenscale << (siglenscale_default ? " (default)" : "")
       << ", sigpascale=" << config.sigpascale << (sigpascale_default ? " (default)" : "")
       << ", exptime=" << config.exptime << (exptime_default ? " (default)" : "") << " s\n";
  ...
  for(long ni = 0; ni < nnights; ni++) {
    ...
    int st = make_trailed_tracklets2(detvec_night, img_log_night, config, pd_night, trk_night, t2d_night);
    night_status[ni] = st;
    if(st != 0) {
      cerr << "ERROR: make_trailed_tracklets2 failed with status " << st << " on night index " << ni << "\n";
      continue;
    }
