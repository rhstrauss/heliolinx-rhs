// March 26, 2026: link_purify_chisq:
// Like link_purify, but calculates chi-square values using resolved
// along-track vs. cross-track astrometric uncertainties.
//
// Description of related program link_purify:
...
int main(int argc, char *argv[])
{
  ...
  status = link_purify_chisq(image_log, detvec, clustvecmain, clust2detmain, config, outclust, outclust2det);
  if(status!=0) {
    cerr << "ERROR: link_purify_chisq failed with status " << status << "\n";
    return(status);
  } 
