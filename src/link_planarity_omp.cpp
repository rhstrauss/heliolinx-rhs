// link_planarity_omp.cpp: March 2026
// OpenMP-parallelized wrapper for link_planarity_omp().
// Identical interface to link_planarity, but the per-cluster processing
// loop (planarity check + iterative culling + Herget orbit fit) runs in
// parallel across all available OMP threads.  Use this after
// heliolinc_lowmem_omp, where all hypotheses land in one set of output
// files and must be processed together for proper cross-hypothesis
// deduplication.
